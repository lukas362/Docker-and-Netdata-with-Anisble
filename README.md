...
```bash
How to run - ansible-playbook docker.yml -K
(-K = Ansible asks for your sudo password to run this playbook. Needed because Docker needs root access)
```

```bash
How to stop - sudo systemctl stop docker
```

```bash
Access Netdata monitor: http://localhost:19999
```
